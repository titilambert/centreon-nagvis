...
class CoreAuthModCentreon extends CoreAuthModule {
	...
	public function __construct() {
		...
		$this->authlog = cfg('authmysqldb', 'authlog');
		
		$this->DB = new CoreCentreonHandler();
		
		// Open mysql database
		if(!$this->DB->open(
                                        cfg('authmysqldb', 'host'),
                                        cfg('authmysqldb', 'port'),
                                        cfg('authmysqldb', 'user'),
                                        cfg('authmysqldb', 'password'),
                                        cfg('authmysqldb', 'database')
                        )) {
			new GlobalMessage(
                                'ERROR', GlobalCore::getInstance()->getLang()->getText('Unable to open auth database ([DB])',
                                Array('DB' => cfg('authmysqldb', 'database')))
                        );
		}
		...
		if(isset($_GET["centreonnagvis"])){
			if (isset($_SESSION["centreonnagvis"])){
				...
				if($cursess != $_GET["centreonnagvis"]){
					...
					session_destroy();
					//$this->cnlog('Location:'.CoreRequestHandler::getRequestUri(cfg('paths', 'htmlbase')));									
